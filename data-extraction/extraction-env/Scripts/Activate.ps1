...
$ReqFile = Join-Path -Path $VenvDir -ChildPath "..\requirements.txt"
if (Test-Path $ReqFile) {
    pip install --quiet -r $ReqFile
    Write-Output "Dependencies are installed!"
}

# If a postactivate script exists, dot-source it so it actually runs in this session
$postActivateFile = Join-Path $PSScriptRoot "postactivate.ps1"
if (Test-Path $postActivateFile) {
    . $postActivateFile
}
